...
inline int photonDetector (double oTime, double dFreq, double dFlux, double dSYNFlux, double dSYNdisordFlux, double dSSCFlux, double dECDFlux, double dBLRFlux, double dDTFlux, double dUPPrevFlux, double dDOPrevFlux) {
	...
	if(item != NULL) {
		...
		if(ff2 != NULL) {
		  ...
		  ff2->dSYNdisordFlux += dSYNdisordFlux;
		...
		ff->dSYNdisordFlux = dSYNdisordFlux;
...
inline int sendToDetector (int zone, int oTime, double aaPhotonDensity[FREQ_GRID], double dVolCoeff, int acclerationFlag, double *nuFnu_max, char *sOutputDirectory, char *nfnfile, double aaSYNDensity[FREQ_GRID], double aaSYNdisordDensity[FREQ_GRID], double aaSSCDensity[FREQ_GRID], double aaECDDensity[FREQ_GRID], double aaBLRDensity[FREQ_GRID], double aaDTDensity[FREQ_GRID], double aaUPPrevDensity[FREQ_GRID], double aaDOPrevDensity[FREQ_GRID], double angle, int timecountfsrs) {
	...
	double aUPFlux[FREQ_GRID], aDOFlux[FREQ_GRID], aSYNdisordFlux[FREQ_GRID];
	...
	for (j = 0; j < FREQ_GRID; j++) {
		...
		aSYNdisordFlux[j] = dVolCoeff * SQR(syn_nu[j]) * aaSYNdisordDensity[j];
		...
		photonDetector(oTime, (D_Z * syn_nu[j]), max(1.0e-40, aFlux[j]), max(1.0e-40, aSYNFlux[j]), max(1.0e-40, aSYNdisordFlux[j]), max(1.0e-40, aSSCFlux[j]), max(1.0e-40, aECDFlux[j]), max(1.0e-40, aBLRFlux[j]), max(1.0e-40, aDTFlux[j]), max(1.0e-40, aUPFlux[j]), max(1.0e-40, aDOFlux[j]));		
...
inline int sendToDetector2 (int zone, int oTime, double aaSiPhotonDensity[FREQ_GRID], double aaPhotonDensity[FREQ_GRID], double dVolCoeff, int acclerationFlag, double *nuFnu_max, char *sOutputDirectory, char *nfnfile, double iZoneNumber, double aaSiSYNDensity[FREQ_GRID], double aaSYNDensity[FREQ_GRID], double aaSiSYNdisordDensity[FREQ_GRID], double aaSYNdisordDensity[FREQ_GRID], double aaSiSSCDensity[FREQ_GRID], double aaSSCDensity[FREQ_GRID], double aaSiECDDensity[FREQ_GRID], double aaECDDensity[FREQ_GRID], double aaSiBLRDensity[FREQ_GRID], double aaBLRDensity[FREQ_GRID], double aaSiDTDensity[FREQ_GRID], double aaDTDensity[FREQ_GRID], double aaSiUPPrevDensity[FREQ_GRID], double aaUPPrevDensity[FREQ_GRID], double aaSiDOPrevDensity[FREQ_GRID], double aaDOPrevDensity[FREQ_GRID], int timecountfsrs) {
	...
	double aEscBLRFlux[FREQ_GRID], aEscDTFlux[FREQ_GRID], aSYNdisordFlux[FREQ_GRID];
	double aEscUPFlux[FREQ_GRID], aEscDOFlux[FREQ_GRID], aEscSYNdisordFlux[FREQ_GRID];
	...
	for (j = 0; j < FREQ_GRID; j++) {
		...
		aSYNdisordFlux[j] = dVolCoeff * SQR(syn_nu[j]) * aaSiSYNdisordDensity[j];
		...
		if (zone == (Ntot - 1) && (acclerationFlag == FALSE)) {
			...
			aEscSYNdisordFlux[j] = dVolCoeff * iZoneNumber * SQR(syn_nu[j]) * aaSYNdisordDensity[j];
			...
			aSYNdisordFlux[j] += aEscSYNdisordFlux[j];
...
void displayTelescopeData () {
    ...
    for(item = map; item != NULL; item = item->hh.next) {
		...
		for(ff = item->aFreqFlux; ff != NULL; ff = ff->hh.next) {
                  ...
                  printf("\n\tdFreq = %e & dFlux = %e, dSYNFlux = %e, dSYNdisordFlux = %e, dSSCFlux = %e, dECDFlux = %e, dBLRFlux = %e, dDTFlux = %e, dUPPrevFlux = %e, dDOPrevFlux = %e", ff->dFreq, ff->dFlux, ff->dSYNFlux, ff->dSYNdisordFlux, ff->dSSCFlux, ff->dECDFlux, ff->dBLRFlux, ff->dDTFlux, ff->dUPPrevFlux, ff->dDOPrevFlux);
    ...
    for(item = map; item != NULL; item = item->hh.next) {
		...
		if(fp) {
			...
			for(ff = item->aFreqFlux; ff != NULL; ff = ff->hh.next) {
			  ...
			  fprintf(fp, "%e %e %e %e %e %e %e %e %e %e\n", ff->dFreq, ff->dFlux, ff->dSYNFlux, 
				  ff->dSYNdisordFlux, ff->dSSCFlux, ff->dECDFlux, ff->dBLRFlux, ff->dDTFlux, 
				  ff->dUPPrevFlux, ff->dDOPrevFlux);
